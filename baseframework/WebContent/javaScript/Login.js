...
$(function () {
    ...
    if ($('#screenOverLoading').length > 0) {
        Public.tips({type: 1, content: '登陆超时，请重新登陆！'});
...
//	}
    ...
    Public.ajax(web_app.name + '/login.ajax', params, function (result) {
        if (result) {
            switch (result.status) {
                ...
                case LoginStatus.SUCCESS:
                    ...
                    if(window.location.href.indexOf('ssoAction!getLoggedInUser.do')!=-1){
                        //单点登录成功，获取登录用户信息
