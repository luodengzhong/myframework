...
PersonalPasswordAuth.showScreenOver=function(){
	var screenOver=$('#pay-authentication-screen-over');
	if(!screenOver.length){
		screenOver=$('<div id="pay-authentication-screen-over" style="position:absolute;top:0px;left:0px;width:0;height:0;z-index:10000;display:none;"></div>').appendTo('body');
	}
	var d = $(document),  h=d.height(), mt = d.scrollTop();
...
PersonalPasswordAuth.hideScreenOver=function(){
	$('#pay-authentication-screen-over').hide();
};
//显示密码输入对话框
PersonalPasswordAuth.showDialog=function(op){
	...
	html.push('<input type="password" id="pay-password-input" style="width:150px;background: none repeat scroll 0 0 #F4F4F4;border: 1px solid #878787;line-height: 16px;padding:4px;color:#999;" value=""/>');
	html.push('</div>');
	...
	var options={
		...
		onClick:function($el){
			if($el.hasClass('ok')){//确定按钮
				PersonalPasswordAuth.authentication(this,okFun);
...
//进行密码验证
PersonalPasswordAuth.authentication=function(obj,fn){
	var password=$('#pay-password-input').val();
	if(password==''){
		Public.tip('请输入访问密码!');
		return;
	}
	Public.ajax(PersonalPasswordAuth.url, {password:password}, function(data) {
		if($.isFunction(fn)){
			fn.call(obj,data);
		}
	});
