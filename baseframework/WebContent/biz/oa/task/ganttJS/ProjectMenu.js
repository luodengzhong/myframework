var EditMenu = function () {
    EditMenu.superclass.constructor.call(this);
};

mini.extend(EditMenu, mini.Menu, {
    _create: function () {
        EditMenu.superclass._create.call(this);
...
mini.extend(ViewMenu, mini.Menu, {
    _create: function () {
        EditMenu.superclass._create.call(this);

        var menuItems = [
            { type: "menuitem", iconCls: "icon-goto", text: '查看', name: "goto" },
            { type: "menuitem", iconCls: "icon-edit", text:'调整计划', name: "applyEdit" },
            '-',
            { type: "menuitem", iconCls: "icon-add", text:'新增计划', name: "addPlan" },
            { type: "menuitem", iconCls: "icon-expand", text:'汇报进度', name: "progressReporting" }            
        ];
        this.setItems(menuItems);

        this.goto = mini.getbyName("goto", this);
        this.addPlan = mini.getbyName("addPlan", this);
        this.progressReporting = mini.getbyName("progressReporting", this);
        this.applyEdit = mini.getbyName("applyEdit", this);
        this.goto.on("click", this.__OnGoto, this);
        this.addPlan.on("click", this.__OnAddPlan, this);
        this.progressReporting.on("click", this.__OnProgressReporting, this);
        this.applyEdit.on("click", this.__applyEdit, this);
    },
    __OnGoto: function (e) {
        showViewTask();
    },
    __OnAddPlan: function (e) {
        addPlan();
    },
    __OnProgressReporting: function (e) {
        progressReporting();
    },
    __applyEdit:function(e){
    	applyEdit();
    }
});