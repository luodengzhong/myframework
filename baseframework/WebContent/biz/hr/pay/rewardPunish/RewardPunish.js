...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/hRrewardPunishAction!slicedQueryRewardPunish.ajax',
		manageType:'hrRewardPunishManage',
		pageSize : 20,
		width : '100%',
		height : '100%',
...
function deleteHandler(){
	...
	UICtrl.confirm('确定删除吗?',function(){
		Public.ajax(web_app.name + '/hRrewardPunishAction!deleteRewardPunish.ajax',{auditId:row.auditId}, function(){
			reloadGrid();
		});
	});
}
function viewHandler(auditId){
	...
	var url=web_app.name + '/hRrewardPunishAction!showUpdateRewardPunish.job?bizId='+auditId+'&isReadOnly=true';
	parent.addTabItem({ tabid: 'HRRewardPunishView'+auditId, text: '查看制度奖罚审批表', url:url});
}