...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/hRCommonWageDataAction!slicedQueryCommonWageData.ajax',
		manageType:'hRDeductManage',
		pageSize : 20,
		width : '100%',
		height : '100%',
...
function deleteHandler(){
	...
	UICtrl.confirm('确定删除吗?',function(){
		Public.ajax(web_app.name + '/hRCommonWageDataAction!deleteCommonWageData.ajax',{auditId:row.auditId}, function(){
			reloadGrid();
		});
	});
}
...
function viewHandler(auditId){
	...
	var url=web_app.name + '/hRCommonWageDataAction!showUpdate.job?bizId='+auditId+'&isReadOnly=true';
	parent.addTabItem({ tabid: 'HRCommonWageData'+auditId, text: '查看通用工资数据审批表', url:url});
}