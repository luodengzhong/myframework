...
$(document).ready(function() {
	...
	compensateKind=$('#mainCompensateKind').combox('getJSONData');
	initializeGrid();
	initializeUI();
});
function initializeUI(){
	...
	PayPublic.updateOperationPeriodByAudit(gridManager,'hRDeductAction!updateOperationPeriod.ajax');
	$('#compensationDeductFileList').fileList();
}
//初始化表格
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		{ display: "类别", name: "compensateKind", width: 100, minWidth: 60, type: "string", align: "left",
			editor: { type:'combobox',data:compensateKind,required: true},
			render: function (item) { 
				return compensateKind[item.compensateKind];
			}
		},	
		...
		dataAction : 'server',
		url: web_app.name+'/hRDeductAction!slicedQueryCompensationDeductDetail.ajax',
		...
		totalRender: function(data){
			if(!data) return '';
			var html=['合计: '];
			$.each(compensateKind,function(k,v){
...
function setId(value){
	...
	$('#compensationDeductFileList').fileList({bizId:value});
}
function afterSave(){
	reloadGrid();
}
...
function deleteHandler(){
	DataUtil.delSelectedRows({action:'hRDeductAction!deleteCompensationDeductDetail.ajax',
...
function saveImpHandler(){
	...
	UICtrl.showAssignCodeImpDialog({title:'导入补发补扣',serialId:serialId,templetCode:'hrCompensationDeductImp'});
}