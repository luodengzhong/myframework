...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/attBaseInfoAction!slicedQueryHolidayOnduty.ajax',
		parms: { year: getYear() },
		pageSize : 20,
		width : '100%',
		height : '100%',
...
function onFolderTreeNodeClick(data) {
	...
	if(data){
		...
		html.push('<font style="color:Tomato;font-size:13px;">[',name,']</font>节假日上班列表');
		//鉴权通过后才能执行
...
function saveHandler(){
	...
	Public.ajax(web_app.name +'/attBaseInfoAction!saveHolidayOnduty.ajax',
		{
...
function deleteHandler(){
	DataUtil.delSelectedRows({ action:'attBaseInfoAction!deleteHolidayOnduty.ajax', 
		gridManager: gridManager, idFieldName: 'holidayOndutyId',
		onSuccess:function(){
			reloadGrid();
		}
	});
