...
function onFolderTreeNodeClick(data) {
	...
	if(!data){
		...
		html.push('<font style="color:Tomato;font-size:13px;">[',fullName,']</font>培训变更申请列表');
	}
...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		{ display: "变更原因", name: "changeReason", width: 100, minWidth: 60, type: "string", align: "left" }
		],
		dataAction : 'server',
		url: web_app.name+'/trainingChangeApplyAction!slicedQuery.ajax',
		pageSize : 20,
		width : '100%',
		height : '100%',
		heightDiff : -10,
		...
		onDblClickRow : function(data, rowindex, rowobj) {
			viewHandler(data.changeApplyId);
		}
	});
...
//编辑按钮
function viewHandler(changeApplyId){
	if(!changeApplyId){
		var row = gridManager.getSelectedRow();
		if (!row) {Public.tip('请选择数据！'); return; }
		changeApplyId=row.changeApplyId;
	}
	parent.addTabItem({
		tabid: 'HRTrainChangeApplyList'+changeApplyId,
		text: '培训变更申请明细',
		url: web_app.name + '/trainingChangeApplyAction!showUpdate.do?bizId=' 
			+ changeApplyId+'&isReadOnly=true'
	}
	);
}
...
function saveSortIDHandler(){
	var action = "trainingChangeApplyAction!updateSequence.ajax";
...
function enableHandler(){
	DataUtil.updateById({ action: 'trainingChangeApplyAction!updateStatus.ajax',
