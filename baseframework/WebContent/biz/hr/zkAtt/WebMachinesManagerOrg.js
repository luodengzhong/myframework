...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/zkAttAction!slicedQueryWebMachinesManagerOrg.ajax',
		parms:{macSnId:$('#macSnId').val()},
		pageSize : 20,
		width : '100%',
		height : '100%',
		heightDiff : -5,
...
function deleteHandler(){
	DataUtil.del({action:'zkAttAction!deleteWebMachinesManagerOrg.ajax',
    ...
    if (!selectOrgDialog) {
        selectOrgDialog = UICtrl.showDialog({
            ...
            ok: function(){
			    ...
			    params.macSnId=$('#macSnId').val();
			    Public.ajax(web_app.name + "/zkAttAction!saveWebMachinesManagerOrg.ajax", params, function () {
			    	reloadGrid();
