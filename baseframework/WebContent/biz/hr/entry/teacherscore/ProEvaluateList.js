...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		{ display: "处理人", name: "doPerson", width: 100, minWidth: 60, type: "string", align: "left" }	   
		...
		onDblClickRow : function(data, rowindex, rowobj) {
			updateHandler(data.teacherScoreId);
		}
	});
...
//编辑按钮
function updateHandler(teacherScoreId){
	if(!teacherScoreId){
	...
	parent.addTabItem({
		tabid: 'HRProEvaluate'+teacherScoreId,
		text: '新员工试用期评价表',
		url: web_app.name + '/teacherScoreAction!showUpdatePro.job?bizId=' 
			+ teacherScoreId
	});
