...
<html>
  <head>
  	<x:base include="dialog,grid,dateTime,combox,tree"/>
		<script src='<c:url value="/biz/hr/recruit/recruitapply/RecruitApplyBillEnd.js"/>' type="text/javascript"></script>
  ...
  <body>
  	<div class="ui-form" style="width: 99%;">
		...
		<x:inputTD name="recDptName" required="true" label="招聘部门"  wrapper="select"/>
		<x:hidden name="recPosId" />			
		<x:inputTD name="recPosName" required="true" label="招聘岗位"  wrapper="select"/>	
		...
		<tr>
                ...
                <x:inputTD name="workDate" required="true" label="要求上岗时间" wrapper="date"/>	
				<x:hidden name="staffKind"/>						
		</tr>
		<tr>
						<x:inputTD name="beRecNum" required="true" label="已招聘人数"  readonly="true"/>	
					  <td colspan="4"  id="infoshow" style="color:red;font-size:14px"></td>
						
		</tr>
		<tr>
			<x:textareaTD name="reason" required="true" label="招聘原因"
						rows="3" colspan="5" />
		</tr>
		</table>
		...
		<tr>
            ...
            <x:inputTD name="eduReq" required="true" label="学历要求"  maxLength="32"/>	
            <x:inputTD name="ageReq" required="true" label="年龄要求" maxLength="16"/>					
            <td colspan=2 class='title'></td>
   		</tr>
		<tr>
				<x:inputTD name="yearReq" required="true" label="工作经验" maxLength="32"/>					
				<x:inputTD name="address" required="true" label="户籍属地" maxLength="64"/>	
				 <td colspan=2 class='title'></td>				
		
		</tr>
		<tr>				
		<x:textareaTD name="professionalReq" required="true" label="专业资格" colspan="5"  maxLength="100"/>					
		</tr>
		<tr>
		...
		<tr>
			<x:textareaTD name="otherReq" required="true" label="其他要求"
		...
		<x:title title="其他" name="group"/>
				<table class='tableInput' id='queryTable'>
		          <x:layout proportion="100px,160px,80px,160px,80px,160px" />
		            ...
		            <x:selectTD name="isHeadhunter"  dictionary="yesorno"
						label="猎头是否可见" />
						<x:selectTD name="isSchoolrecruit" dictionary="yesorno" label="是否校园招聘" />
