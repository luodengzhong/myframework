...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/parameterAction!slicedQueryMessageKind.ajax',
		pageSize : 20,
		width : '99.5%',
		height : '100%',
		heightDiff : -10,
...
function addHandler() {
	UICtrl.showAjaxDialog({
		url: web_app.name + '/parameterAction!showInsertMessageKind.load',
		title: "添加消息发送类别",
		width:400, 
		ok: saveMessageKind, 
		close: dialogClose
...
function updateHandler(messageKindId){
	...
	UICtrl.showAjaxDialog({
		url: web_app.name + '/parameterAction!showUpdateMessageKind.load',
		title: "修改消息发送类别",
		width:400, 
...
function deleteHandler(){
	DataUtil.del({action:'parameterAction!deleteMessageKind.ajax',
...
function saveMessageKind() {
	$('#submitForm').ajaxSubmit({url: web_app.name + '/parameterAction!saveMessageKind.ajax',
		success : function(id) {
...
function enableHandler(){
	DataUtil.updateById({ action: 'parameterAction!updateMessageKindStatus.ajax',
