...
$(function () {
    ...
    function loadFunctionTree() {
        ...
        Public.ajax(web_app.name + "/permissionAction!queryFunctions.ajax", {
        	parentId: functionId,
        	id: functionId,
    ...
    function onBeforeExpand(node) {
        if (node.data.hasChildren) {
            if (!node.data.children || node.data.children.length == 0) {
                Public.ajax(web_app + "/permissionAction!queryFunctions.ajax", {
    ...
    function initializeRoleGrid() {
        roleGridManager = UICtrl.grid("#roleGrid", {
            ...
            dataAction: 'server',
            url: web_app.name
					+ "/permissionAction!queryRolesByFunctionId.ajax",
            usePager: false,
            title: "分配角色列表",
            width: '49%',
            height: '99%',
    ...
    function initializeOrgGrid() {
        orgGridManager = UICtrl.grid("#orgGrid", {
            ...
            dataAction: 'server',
            url: web_app.name
					+ "/permissionAction!queryOrgsByFunctionId.ajax",
            usePager: false,
            title: "分配组织列表",
            width: '49%',
            height: '99%',
