...
$(function() {
	...
	function bindEvents(){
		$("#checkPermission").on('click', function (e) {
	        ...
	        if ($clicked.is("a.tLink")) {
	            ...
	            if (kindId == "fun"){
	            	url = "permissionAction!forwardQueryPermissionForFunction.do";
	            	tabId = "queryPermissionForFunction";
	            }else{
	            	url = "permissionAction!forwardQueryPermissionForOrg.do"; 
	            }
	            url+="?funId="  + getFunctionId() + "&inOrgFullId=" +getScopeOrgFullId();
	            parent.addTabItem({ tabid: tabId, text: $clicked.text(), url: url});
	...
	function initData() {
		...
		Public.ajax(web_app.name + '/permissionCheckAction!queryHandler.ajax',
