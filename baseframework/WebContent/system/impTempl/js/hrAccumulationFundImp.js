var imp_log_query_manage_type = 'hrAccumulationFundImp';
...
function getLogGridToolbarOptions() {
	...
	tmp_button.comboDialog({
		...
		onChoose : function() {
			...
			Public.ajax(web_app.name+ '/staffWelfareAction!updateAccumulationFundOperationPeriodBySerialId.ajax',{periodId : periodId,serialId : choose_serialId});
			return true;
		}
	});
	return UICtrl.getDefaultToolbarOptions({
			...
			click : function() {
				...
				UICtrl.confirm('确定删除本次导入数据吗?', function() {
					Public
							.ajax(
									web_app.name
											+ '/staffWelfareAction!deleteAccumulationFundBySerialId.ajax',
									{
										serialId : serialId
									}, function() {
										reloadGrid();
		...
		noProvidentFundPerson : {
			id : 'noProvidentFundPerson',
			text : '查询导入遗漏人员',
			img : 'page_find.gif',
			click : function() {
				var row = gridManager.getSelectedRow();
		...
		saveManualOperation : {
			...
			click : function() {
				...
				Public
						.ajax(
								web_app.name
										+ '/staffWelfareAction!saveRepeatImpAccumulationFund.ajax',
								{
									ids : $.toJSON(ids),
									serialId : serialId
								}, function() {
