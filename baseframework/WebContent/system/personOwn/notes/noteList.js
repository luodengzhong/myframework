...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/personOwnAction!slicedQueryPersonNotes.ajax',
		pageSize : 20,
		sortName:'id',
		sortOrder:'desc',
		width : '100%',
...
function add(){
	UICtrl.showAjaxDialog({url:web_app.name+'/personOwnAction!toAddPage.load',title:'新增便签',ok: addNote,width:420,init:init_add_dialog});
}

//删除按钮
function deleteHandler(){
	DataUtil.del({action:'personOwnAction!delNote.ajax',
...
function delAll() {
	UICtrl.confirm("此操作不能回退，确信要删除全部数据吗？", function() {
		Public.ajax(web_app.name+'/personOwnAction!doDeleteAll.ajax', {}, function() {
			reloadGrid();
		});
	});
}
...
function addNote(){
	...
	Public.ajax(web_app.name+'/personOwnAction!addNote.ajax', data, function(msg) {
		reloadGrid();
		_self.close();
	});
...
//返回图形显示页面
function goPicView(){
	window.location.href=web_app.name+'/personOwnAction!toMainPage.do?timeStamp='+ new Date().getTime();
}