...
$(document).ready(function() {
  ...
  function initializeTaskGrid() {
		var toolbarParam = { 
		  updateHandler: { id: "updateTask", text: "更改处理人", click: updateTaskHandler, img: "page_edit.gif" },
		  updateTaskReadyHandler: { id: "ready", text: "未处理", click: updateTaskStatusHandler, img: "flag_red.gif" },
		  updateTaskCompletedyHandler: { id: "completed", text: "已完成", click: updateTaskStatusHandler, img: "flag_blue.gif" },
		  updateTasksDescriptionHandler: { id: "updateTasksDescription", text: "修改主题", click: updateTasksDescription, img: "page_edit.gif"},
		  transferHandler: { id: "transfer", text: "交办", click: transfer, img: "page_user.gif"},
		  handTasksHandler: { id: "handTasks", text: "交接任务", click: handTasks, img: "page_user.gif"},
		  timingProcessHandler: { id: "timingProcess", text: "计时处理", img: "page_user.gif" }
		};
		...
		taskGridManager = UICtrl.grid("#taskGrid", {
			...
			dataAction : "server",
			url : web_app.name + "/workflowAction!queryHiTaskInstByBizCode.ajax",
			width : "99.8%",
			height : '100%',
			usePager : false,
			...
			onDblClickRow: function (data, rowindex, rowobj) {
			    ...
			    parent.addTabItem({ tabid: data.id, text: data.name, url: data.executorUrl + params });  
		   },
		   onSelectRow: function(data, rowid, rowobj){
		   	  var toolbarManager = taskGridManager.toolbarManager;
		   	  if (TaskStatus.isReayStatus(data.statusId)){
		   	      toolbarManager.setEnabled("menutransfer");
		   	      toolbarManager.setEnabled("menuupdateTask");
		   	      toolbarManager.setDisabled("menuready");
		   	      toolbarManager.setEnabled("menucompleted");
		   	  }else{
		   	      toolbarManager.setDisabled("menutransfer");
		   	      toolbarManager.setDisabled("menuupdateTask");
		   	      toolbarManager.setEnabled("menuready");
		   	      toolbarManager.setDisabled("menucompleted");
		   	  }
		   }
		});
	...
	function getHIProcUnitHandlerGridOptions(){
		return  {		
			columns : [ 
                ...
                { name : "handleKindName", display : "处理环节", width : 100, minWidth : 60, type : "string", align : "left" },
			...
			dataAction : "server",
			url : web_app.name + "/flowTaskHandlerManageAction!queryHistoricProcUnitHandler.ajax",
			checkbox: true,
			usePager : false,
			width : "99.8%",
			height : "100%",
	...
	function getProcUnitHandlerGridOptions(){
		...
		mendHandler: { id: "launchMendTask", text: "发起补审任务", click: launchMendTask, img: "action_go.gif" },
		...
		return {		
			columns : [ 
                ...
                { name : "handlerName", display : "人员成员", width : 100, minWidth : 60, type : "string", align : "left",
				   ...
				   getParam: function(){
					...
					id: "handlerId", personMemberName: "handlerName" }
			...
			dataAction : "server",
			url : web_app.name + "/flowTaskHandlerManageAction!queryProcunitHandlerByBizCode.ajax",
			checkbox: true,
			usePager : false,
			toolbar : toolbarOptions,
			width : "99.8%",
...
function updateTaskStatusHandler(item){
	...
	params.status = item.id.substring(4);
	...
	Public.ajax(web_app.name + "/workflowAction!updateHistoricTaskInstanceStatus.ajax",
		params, function() {
			_self.close();
...
function doUpdateTaskHandlerHandler(taskId) {
	...
	Public.ajax(web_app.name
							+ "/flowTaskHandlerManageAction!updateFlowTaskHandler.ajax",
					params, function() {
						//parent.Public.successTip("您已成功更改流程任务处理人。");
						_self.close();
						$("#btnQueryTask").click();
					});
}
...
function doUpdateProcUnitHandlerHandler(procUnitHandlerId) {
	...
	Public.ajax(
					web_app.name
							+ "/flowTaskHandlerManageAction!updateFlowTaskHandler.ajax",
					params, function() {
						//parent.Public.successTip("您已成功更改环节处理人。");
						_self.close();
...
function saveProcUnitHandler() {
	...
	Public.ajax(web_app.name + '/workflowAction!saveProcUnitHandler.ajax', {
				data : encodeURI($.toJSON(procUnitHandlerData))
			}, function() {
				procUnitHandlerGridManager.loadData();
			});
}
...
function deleteProcUnitHandler() {
	...
	for (var i = 0; i < data.length; i++) {
		if (data[i].status != 0) {
			Public.errorTip("处理环节”" + data[i].handleKindName
					+ "“状态不为”未处理“，不能删除。");
			return;
		}
	}
	
	var action = '/workflowAction!deleteProcUnitHandler.ajax';
...
function launchMendTask(){
	...
	Public.ajax(web_app.name
							+ "/workflowAction!launchMendTask.ajax",
					params, function() {
...
function updateTasksDescription(){
	...
	Public.ajax(web_app.name + '/workflowAction!updateTasksDescription.ajax', {data: params.toString() }, function(data) {
...
function doHandTasks(){
    ...
    var _self = this;
    Public.ajax(web_app.name + "/workflowAction!handTasks.ajax", params, function () {
             _self.close();
    });
}
...
function signNeedTimeHandler(needTiming){	
	...
	//3、call service
	Public.ajax(web_app.name + '/workflowAction!updateTaskInstExtensionNeedTiming.ajax', param);
}
...
function doMisOperate(kindId){
	...
	Public.ajax(web_app.name + '/workflowAction!' + action + '.ajax',  { bizCode: bizCode });
}
