...
function initializeGrid() {
	...
	gridManager = UICtrl.grid('#maingrid', {
		...
		dataAction : 'server',
		url: web_app.name+'/suggestionBoxAction!slicedQuery.ajax',
		pageSize : 20,
		width : '100%',
		height : '100%',
		heightDiff : -5,
...
function updateHandler(boxId){
	...
	UICtrl.showAjaxDialog({url: web_app.name + '/suggestionBoxAction!showUpdate.load', param:{boxId:boxId},width:500, ok: update, close: dialogClose});
}
...
function deleteHandler(){
	...
	UICtrl.confirm('确定删除吗?',function(){
		//所需参数需要自己提取 {id:row.id}
		Public.ajax(web_app.name + '/suggestionBoxAction!delete.ajax', {boxId:row.boxId}, function(){
			reloadGrid();
		});
	});
}
...
//编辑保存
function update(){
	$('#suggestionBoxForm').ajaxSubmit({url: web_app.name + '/suggestionBoxAction!update.ajax',
		success : function() {
			refreshFlag = true;
		}
	});
