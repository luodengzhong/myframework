...
function loadFolderTree(options) {
	...
	Public.ajax(web_app.name + '/configurationAction!loadCommonTrees.ajax',
				{ kindId : p.kindId },
				function(data) {
	...
	if (node.data.hasChildren) {
		if (!node.data.children || node.data.children.length == 0) {
			Public.ajax(web_app.name
					+ '/configurationAction!loadCommonTrees.ajax', {
...
function addFolder() {
	UICtrl.showAjaxDialog({
		url : web_app.name
				+ "/configurationAction!forwardCommonTreeDetail.load",
...
function updateFolder() {
	UICtrl.showAjaxDialog({
		url : web_app.name + "/configurationAction!loadCommonTree.load",
		title : "修改文件夹",
		width : 400,
		ok : updateCommonTree,
		close : commonTreeDialogCloseHandler
...
function insertCommonTree(doc) {
	...
	$('form:first',doc).ajaxSubmit({
		url : web_app.name + '/configurationAction!insertCommonTree.ajax',
		param : {
			parentId : folderId,
			kindId : kindId
		},
...
function updateCommonTree(doc) {
	var _self = this;
	$('form:first',doc).ajaxSubmit({
		url : web_app.name + '/configurationAction!updateCommonTree.ajax',
		param : {
			id : folderId
		},
...
function deleteFolder(folderId) {
	UICtrl.confirm('确定删除吗?', function() {
		Public.ajax(
				web_app.name + '/configurationAction!deleteCommonTree.ajax', {
	...
	if (folderTreeManager) {
		...
		if (node) {
			...
			if (prevData){
				...
				Public.ajax(web_app.name + '/configurationAction!updateCommonTreeSequence.ajax',{data:params.toString()} ,
						function(){
					Public.tip('上移文件夹成功!');
...
/* 扩展属性 */
CommonTreeKind.ExtendedField = 6;
/* 指标模板 */
