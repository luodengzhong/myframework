...
$(document).ready(function() {
	...
	UICtrl.autoSetWrapperDivHeight(function(pageSize){
		$('#divTreeArea').height(pageSize.h -92);
	});
	initializeUI();
	bindEvent();
});
...
function viewAttachment(id,name,fileKind){
	...
	if(AttachmentUtil.isImgForThickbox(fileKind)){
		var src=web_app.name+'/attachmentAction!downFile.ajax?id='+id;
...
function downFile(){
	...
	var url=web_app.name+'/attachmentAction!downFile.ajax?a=1&id='+id+'&a='+new Date().getTime();
	$('#downFileFrame')[0].src=url;
}