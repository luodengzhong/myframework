...
JQWebUploader.createWebUploader=function(element){
	...
	if(!uploader){
		...
		//设置对应的上传按钮对象[  WebUploader 无此方法 webuplpader.js 1934行添加方法 ]
		...
		uploader.on("fileQueued", function(file){
			...
			var html=['<div class="JQWebUploader-title">文件上传:',file.name,'(',JQWebUploader.renderSizeView(file.size),')</div>'];
			html.push('<div><span class="JQWebUploader-item-upload">上传</span><span class="JQWebUploader-item-stop">暂停</span><span class="JQWebUploader-item-del">取消</span><div>');
			html.push('<div class="webuploader-progress"><font style="line-height:20px;">&nbsp;上传数据准备中....</font></div> ');
			div.html(html.join(''));
			//注册事件处理暂停及取消
		...
		uploader.on("uploadProgress", function(file, percentage){
			...
			if(parseInt(percentage,10)==1){
				setTimeout(function(){div.find('span').html("文件合并中...");},1000);
			}
		});
		//上传成功
		...
		uploader.on("error", function(kind){
			if(kind==='F_EXCEED_SIZE'){
				Public.tips({type:1, content : '选择的文件太大("'+JQWebUploader.renderSizeView(1024 * 1024 * 1024)+'")无法上传!'});
			}
		});
