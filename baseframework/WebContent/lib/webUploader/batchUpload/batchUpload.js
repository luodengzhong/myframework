...
JQWebUploader.getBackEndUrl=function(method){
	return web_app.name+'/webUploadAction!'+method+'.ajax';
};
...
function initWebUploader(){
	...
	var uploader = WebUploader.create({
		...
		server: JQWebUploader.getBackEndUrl('ajaxUpload'),
		pick: "#picker",
		resize: false,
	...
	uploader.on("fileQueued",function(file) {
        ...
        html.push('<span class="itemDel">删除</span>');
        if(chekFlag){
              html.push('<div class="percentage"></div>');
        }else{//文件类型校验错误
        	 html.push('<div class="errorMsg">文件类型不正确，无法上传！</div>');
        	 uploader.removeFile(file.id);
        }
        html.push('</li>');
	...
	uploader.on("uploadProgress", function(file, percentage){
		 ...
		 if(parseInt(percentage,10)==1){
			 ...
			 setTimeout(function(){
				...
				div.removeClass('uploader-progress').html("<font color='green'>文件上传成功!</font>");
			 },1000);
		 }
	});
	...
	uploader.on("error", function(kind){
		if(kind==='F_EXCEED_SIZE'){
			Public.tips({type:1, content : '选择的文件太大("'+JQWebUploader.renderSizeView(JQWebUploader.fileSingleSizeLimit)+'")无法上传!'});
		}else if(kind==='Q_EXCEED_NUM_LIMIT'){
			Public.tips({type:1, content : '选择的文件超过允许的个数!'});
		}
	});
...
function onBeforeSendFile(file){
	 ...
	 div.html('<font color="#aaaaaa">上传数据准备中......<font>');
	//控制滚动条位置
...
function initShowUploadInfo(){
	...
	$('#showUploadFileType').html('允许上传文件类型:'+uploadFileType);
	var html=[];
	html.push('允许选择文件<font color=red>[',JQWebUploader.fileNumLimit,'个]</font>,');
	html.push('单文件限制大小<font color=red>[',JQWebUploader.renderSizeView(JQWebUploader.fileSingleSizeLimit),']</font>!');
