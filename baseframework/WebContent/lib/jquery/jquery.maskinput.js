	...
	var maskInput = function (el,mask,settings){
		...
		this.ev={//可用事件
			...
			_blur:function(){
				...
				_self.checkVal();
				if(_self.settings.completed && _self.element.val().length == _self.mask.length)
	...
	$.extend(maskInput.prototype, {
		...
		focusEvent:function(){
			...
			setTimeout(function(obj){
				return function(){
					obj.caret();
					obj.element.trigger('select');//默认全部选中
				};
			}(this),0);
		},
		checkVal:function(){	
		...
		changeMask:function(mask,settings){
			...
			this.firstNonMaskPos=null;
			var newMask=[];
			//循环mask寻找(xrBsMask.charMap中)对应正则表达式
			for(var i=0;i<masks.length;i++){
				var c=masks[i];
				newMask.push(maskCharMap[c]||((/[A-Za-z0-9]/.test(c)?"":"\\")+c));
				this.locked[i]=(maskCharMap[c]==null);
