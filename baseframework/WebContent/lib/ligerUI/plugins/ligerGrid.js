...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 30, 50, 100, 200],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
    ...
    $.ligerDefaults.Grid_columns = {
        ...
        editor: null,
        render: null,
        title:null,//表头提示 xx add  
        sortField:null,//真正用于排序的字段如果设置了再点击排序的是否会传入该值到后台 xx add 2014-12-4
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm)
        {
            var g = this, p = this.options;
            g.loading = true;
            //xx modif  g.trigger('loadData');
            this._onResize();
            if (g.trigger('loadData') == false) return false;
            //end
            var clause = null;
            var loadServer = true;
        ...
        {
        	var jsonParam={};//xiexin 修改传参方式
        	$.each(param,function(i,o){
        		jsonParam[o.name]=o.value;
        	});
        	//结束修改
            var g = this, p = this.options;
            var ajaxOptions = {
                ...
                success: function (d)
                {
                	...
                	Public.ajaxCallback(d,function(data){
                        ...
                        g._showData.ligerDefer(g, 10, [{RecordCount:0,PageIndex:1,PageCount:0}]);
                        g.trigger('error', [data, g]);
                   });
                },
                complete: function ()
                {
        ...
        */
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.display && column.display != "")
                ...
                headerText = "&nbsp;";
            if(column.editor&&p.enabledEdit){//xx add 编辑时表头增加标志
            	headerText='<font color="#2D65DC">'+headerText+'</font>';
            	if(column.editor.required){
            		headerText+='<font color="red">*</font>';
            	}
            }
            $(".l-grid-hd-cell-text:first", jcell).html(headerText);
            if (column.title){
            	$(".l-grid-hd-cell-text:first", jcell).attr('title',column.title);
            }else{
            	$(".l-grid-hd-cell-text:first", jcell).attr('title',$(".l-grid-hd-cell-text:first", jcell).text());
            }
...
//                    }
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        var _index= (p.page-1) * p.pageSize + parseInt(item['__index']) + 1;
                        gridhtmlarr.push('>' + _index + '</div></td>');
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            var title=$('<div></div>').html(content).text();//xx add 默认增加title显示
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                //xx add 参数 rowcell
                if (g.trigger('beforeEdit', [editParm,rowcell]) == false) return false;
                g.lastEditRow = rowdata;
                //xx modif lef 取值方式 增加frozen 判断
                var editor = p.editors[column.editor.type],
                    ...
                    left = pc.left + pb.left + (frozen?0:pv.left),//xx modfi (frozen?0:pv.left)
        ...
        _setEvent: function ()
        {
            ...
            var resizeTimer=null;//xx 防止IE多次执行
            $(window).bind("resize.grid", function (e)
            {	
            	if($.browser.msie&&$.browser.version < 8){//xx 防止IE多次执行
            ...
            {
                try{
                	 if(p.clearScroll===true){
                		 delete p.clearScroll;
                		 return;
                	 }
                     ...
                     if (scrollTop != null){
                         ...
                         if($.browser.msie&&$.browser.version <= 8){//解决行错位问题
	                         var tempScrollTop= g.f.gridbody.scrollTop();
	                         if(tempScrollTop<scrollTop){
	                        	g.f.gridbody.css({top:tempScrollTop-scrollTop,zIndex:0});
	                         }else{
	                         	 g.f.gridbody.css({top:0,zIndex:'auto'});
	                         }
                         }
                     }
                     if(g.isEditorScroll===true){//xx add 编辑时定位处理
                     	delete g.isEditorScroll;
                     	return;
                     }
                     g.endEdit();
                     g.trigger('SysGridHeightChanged');
        ...
        _onContextmenu: function (e)
        {
            ...
            else if (src.hcell)
            {
                ...
                var height=Math.min(g.popup.height(), g.body.height()-20); // xx add min height
                //xx add width
                var width=g.popup.width();
                width=width<10?100:width;//默认 100px
                g.popup.css({ left: left, top: g.gridheader.height() + 1,width:width+10,height:height});
                g.popup.toggle();
                return false;
            }
        },
        ...
        _onClick: function (e)
        {
            ...
            else if (src.hcelltext) //排序
            {
                ...
                //xx add 2014-12-04
                if(src.column.sortField) columnName=src.column.sortField;
        ...
        _onResize: function ()
        {
            ...
            if (p.height && p.height != 'auto')
            {
                ...
                if (typeof (p.height) == "string" && p.height.indexOf('%') > 0)
                {
                    ...
                    //xx add jobPageCenter
                    var jobScrollTop=0;
                    if($('#jobPageCenter').length>0){
                    	jobScrollTop=$('#jobPageCenter').scrollTop();
                    }
                    if (p.inWindow || gridparent[0].tagName.toLowerCase() == "body")
                        h -= (g.grid.offset().top - parseInt($('body').css('paddingTop'))+jobScrollTop);
                }
                else
                {
                    h = parseInt(p.height);
                }
                //alert((g.grid.offset().top - parseInt($('body').css('paddingTop'))));
                h += p.heightDiff;
