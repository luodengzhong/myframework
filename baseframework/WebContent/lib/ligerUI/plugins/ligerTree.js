...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        onError: function () { },
        onClick: function () { },
        onDblclick:function(){},//xx add dblclick
        idFieldName: 'id',
        ...
        */
        delay: null,
        dataRender:null //xx add 数据读取 ajax返回的数据可能需要格式化读取
    };
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //获取节点 数据
        getNodeData: function (treenode) {
            var g = this;
            treenode = g.getNodeDom(treenode);           
            if (!treenode) return null;
            var parentIndex = $(treenode).attr("treedataindex");
            ...
            $.ajax({
                ...
                success: function (d)
                {
                    ...
                    Public.ajaxCallback(d,function(data){
                    	if($.isFunction(p.dataRender)){
                    		data=p.dataRender.call(g,data);
                    	}
                    	g.append(node, data);
        ...
        _getDelay: function (o, level)
        {
            ...
            if (typeof (delay) == "function") delay = delay.call(g,{ data: o, level: level });
            ...
            g.tree.click(function (e)
            {
                ...
                { 
                    if (!treeitem.hasClass("l-unselectable"))
                    {
                        ...
                        {
                        	//xx add param obj
                            if (g.trigger('beforeCancelSelect', [{ data: treenodedata, target: treeitem[0], obj:$(obj) }]) == false)
                                return false;
                        ...
                        {   //xx add param obj
                            if (g.trigger('beforeSelect', [{ data: treenodedata, target: treeitem[0], obj:$(obj) }]) == false)
                                return false;
                ...
                g.trigger('click', [{ data: treenodedata, target: treeitem[0] },obj]);
            });
